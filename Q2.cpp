...
1.const char* str1 = "We love C++ programming\n";
   - str1 points to a string literal `"We love C++ programming\n"`. It points to the first character 'W'.

2.char str2[100];
   - str2 is an array of 100 characters..

3. char *p = str2;
   - p is a pointer that points to the first element of str2. The name of the array str2 is implicitly converted to a pointer to its first element, 
     i.e p now points to str2[0].

4.while(*p++ = *str1++){}
   - This loop copies each character from str1 to str2.
